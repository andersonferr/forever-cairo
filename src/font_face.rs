...
#[inline(always)]
unsafe extern "C" fn _cairo_atomic_int_cmpxchg_return_old_impl(
    mut x: *mut cairo_atomic_int_t,
...
#[no_mangle]
pub static mut _cairo_font_face_nil: cairo_font_face_t = {
...
#[no_mangle]
pub static mut _cairo_font_face_nil_file_not_found: cairo_font_face_t = {
    let mut init = _cairo_font_face {
        hash_entry: {
            let mut init = _cairo_hash_entry {
                hash: 0 as libc::c_int as uintptr_t,
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_font_face_set_error(
    ...
    {} else {
        __assert_fail(
            ...
            b"../src/cairo-font-face.c\0" as *const u8 as *const libc::c_char,
            87 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<
                &[u8; 79],
                &[libc::c_char; 79],
            >(
                b"cairo_status_t _cairo_font_face_set_error(cairo_font_face_t *, cairo_status_t)\0",
            ))
                .as_ptr(),
        );
    }
    ret__ = _cairo_atomic_int_cmpxchg_impl(
        &mut (*font_face).status as *mut cairo_status_t as *mut cairo_atomic_int_t,
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_font_face_init(
    mut font_face: *mut cairo_font_face_t,
    mut backend: *const cairo_font_face_backend_t,
) {
    (*font_face).status = CAIRO_STATUS_SUCCESS;
    (*font_face).ref_count.ref_count = 1 as libc::c_int;
    let ref mut fresh2 = (*font_face).backend;
    *fresh2 = backend;
    _cairo_user_data_array_init(&mut (*font_face).user_data);
}
#[no_mangle]
pub unsafe extern "C" fn cairo_font_face_reference(
    mut font_face: *mut cairo_font_face_t,
) -> *mut cairo_font_face_t {
    if font_face.is_null()
        || _cairo_atomic_int_get(&mut (*font_face).ref_count.ref_count)
            == -(1 as libc::c_int)
    {
        return font_face;
    }
    ::std::intrinsics::atomic_xadd(
        &mut (*font_face).ref_count.ref_count,
        1 as libc::c_int,
    );
    return font_face;
}
#[inline]
unsafe extern "C" fn __put(mut v: *mut cairo_reference_count_t) -> cairo_bool_t {
    let mut c: libc::c_int = 0;
    let mut old: libc::c_int = 0;
    c = _cairo_atomic_int_get(&mut (*v).ref_count);
    while c != 1 as libc::c_int
        && {
            old = _cairo_atomic_int_cmpxchg_return_old_impl(
                &mut (*v).ref_count,
                c,
                c - 1 as libc::c_int,
            );
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_font_face_destroy(
    mut abstract_face: *mut libc::c_void,
) -> cairo_bool_t {
    return 1 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn cairo_font_face_destroy(mut font_face: *mut cairo_font_face_t) {
    if font_face.is_null()
        || _cairo_atomic_int_get(&mut (*font_face).ref_count.ref_count)
            == -(1 as libc::c_int)
    {
    ...
    if _cairo_atomic_int_get(&mut (*font_face).ref_count.ref_count) > 0 as libc::c_int
    {} else {
        __assert_fail(
            b"CAIRO_REFERENCE_COUNT_HAS_REFERENCE (&font_face->ref_count)\0" as *const u8
            ...
            (*::std::mem::transmute::<
                &[u8; 50],
                &[libc::c_char; 50],
            >(b"void cairo_font_face_destroy(cairo_font_face_t *)\0"))
                .as_ptr(),
        );
    }
    if __put(&mut (*font_face).ref_count) != 0 {
        return;
    }
    if ((*(*font_face).backend).destroy)
        .expect("non-null function pointer")(font_face as *mut libc::c_void) == 0
    {
        return;
    }
    _cairo_user_data_array_fini(&mut (*font_face).user_data);
    free(font_face as *mut libc::c_void);
}
#[no_mangle]
pub unsafe extern "C" fn cairo_font_face_get_type(
    mut font_face: *mut cairo_font_face_t,
) -> cairo_font_type_t {
    if _cairo_atomic_int_get(&mut (*font_face).ref_count.ref_count)
        == -(1 as libc::c_int)
    {
        return CAIRO_FONT_TYPE_TOY;
    }
    return (*(*font_face).backend).type_0;
}
#[no_mangle]
pub unsafe extern "C" fn cairo_font_face_get_reference_count(
    mut font_face: *mut cairo_font_face_t,
) -> libc::c_uint {
    if font_face.is_null()
        || _cairo_atomic_int_get(&mut (*font_face).ref_count.ref_count)
    ...
    return _cairo_atomic_int_get(&mut (*font_face).ref_count.ref_count) as libc::c_uint;
}
#[no_mangle]
pub unsafe extern "C" fn cairo_font_face_status(
    mut font_face: *mut cairo_font_face_t,
) -> cairo_status_t {
...
pub unsafe extern "C" fn cairo_font_face_get_user_data(
    ...
    return _cairo_user_data_array_get_data(&mut (*font_face).user_data, key);
}
#[no_mangle]
pub unsafe extern "C" fn cairo_font_face_set_user_data(
    mut font_face: *mut cairo_font_face_t,
    mut key: *const cairo_user_data_key_t,
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_unscaled_font_init(
    mut unscaled_font: *mut cairo_unscaled_font_t,
    mut backend: *const cairo_unscaled_font_backend_t,
) {
    (*unscaled_font).ref_count.ref_count = 1 as libc::c_int;
    let ref mut fresh3 = (*unscaled_font).backend;
    *fresh3 = backend;
}
#[no_mangle]
pub unsafe extern "C" fn _cairo_unscaled_font_reference(
    mut unscaled_font: *mut cairo_unscaled_font_t,
) -> *mut cairo_unscaled_font_t {
    if unscaled_font.is_null() {
        return 0 as *mut cairo_unscaled_font_t;
    }
    if _cairo_atomic_int_get(&mut (*unscaled_font).ref_count.ref_count)
        > 0 as libc::c_int
    {} else {
        __assert_fail(
            b"CAIRO_REFERENCE_COUNT_HAS_REFERENCE (&unscaled_font->ref_count)\0"
            ...
            (*::std::mem::transmute::<
                ...
                &[libc::c_char; 79],
            >(
                b"cairo_unscaled_font_t *_cairo_unscaled_font_reference(cairo_unscaled_font_t *)\0",
            ))
                .as_ptr(),
        );
    }
    ::std::intrinsics::atomic_xadd(
        &mut (*unscaled_font).ref_count.ref_count,
        1 as libc::c_int,
    );
    return unscaled_font;
}
#[no_mangle]
pub unsafe extern "C" fn _cairo_unscaled_font_destroy(
    mut unscaled_font: *mut cairo_unscaled_font_t,
) {
    if unscaled_font.is_null() {
        return;
    ...
    {} else {
        __assert_fail(
            ...
            (*::std::mem::transmute::<
                &[u8; 59],
                &[libc::c_char; 59],
            >(b"void _cairo_unscaled_font_destroy(cairo_unscaled_font_t *)\0"))
                .as_ptr(),
        );
    }
    if __put(&mut (*unscaled_font).ref_count) != 0 {
        return;
    }
    if ((*(*unscaled_font).backend).destroy)
        .expect("non-null function pointer")(unscaled_font as *mut libc::c_void) == 0
    {
        return;
    }
    free(unscaled_font as *mut libc::c_void);
}
