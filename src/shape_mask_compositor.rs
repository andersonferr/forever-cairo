...
unsafe extern "C" fn _cairo_shape_mask_compositor_stroke(
...
unsafe extern "C" fn _cairo_shape_mask_compositor_fill(
    mut _compositor: *const cairo_compositor_t,
    mut extents: *mut cairo_composite_rectangles_t,
    mut path: *const cairo_path_fixed_t,
    ...
    if (*mask).is_clear() == 0 {
        ...
        if status as u64 != 0 {
            current_block = 11971986197738172365;
        } else {
            current_block = 13586036798005543211;
        }
    } else {
...
unsafe extern "C" fn _cairo_shape_mask_compositor_glyphs(
    mut _compositor: *const cairo_compositor_t,
    mut extents: *mut cairo_composite_rectangles_t,
    ...
    if (*mask).is_clear() == 0 {
        ...
        if status as u64 != 0 {
            current_block = 14004877768351354374;
        } else {
            current_block = 13586036798005543211;
        }
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_shape_mask_compositor_init(
    mut compositor: *mut cairo_compositor_t,
    mut delegate: *const cairo_compositor_t,
) {
    let ref mut fresh0 = (*compositor).delegate;
    *fresh0 = delegate;
    let ref mut fresh1 = (*compositor).paint;
    *fresh1 = None;
    let ref mut fresh2 = (*compositor).mask;
    *fresh2 = None;
    let ref mut fresh3 = (*compositor).fill;
    *fresh3 = Some(
    ...
    let ref mut fresh4 = (*compositor).stroke;
    *fresh4 = Some(
    ...
    let ref mut fresh5 = (*compositor).glyphs;
    *fresh5 = Some(
