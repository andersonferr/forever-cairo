...
extern "C" {
    ...
    fn _cairo_pattern_create_in_error(status: cairo_status_t) -> *mut cairo_pattern_t;
    fn _cairo_pattern_init(pattern: *mut cairo_pattern_t, type_0: cairo_pattern_type_t);
}
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_raster_source_pattern_acquire(
    mut abstract_pattern: *const cairo_pattern_t,
    mut target: *mut cairo_surface_t,
    mut extents: *const cairo_rectangle_int_t,
) -> *mut cairo_surface_t {
    let mut pattern: *mut cairo_raster_source_pattern_t = abstract_pattern
        as *mut cairo_raster_source_pattern_t;
    if ((*pattern).acquire).is_none() {
        return 0 as *mut cairo_surface_t;
    }
    if extents.is_null() {
        extents = &mut (*pattern).extents;
    }
    return ((*pattern).acquire)
        .expect(
            "non-null function pointer",
        )(&mut (*pattern).base, (*pattern).user_data, target, extents);
}
#[no_mangle]
pub unsafe extern "C" fn _cairo_raster_source_pattern_release(
    mut abstract_pattern: *const cairo_pattern_t,
    mut surface: *mut cairo_surface_t,
) {
    let mut pattern: *mut cairo_raster_source_pattern_t = abstract_pattern
        as *mut cairo_raster_source_pattern_t;
    if ((*pattern).release).is_none() {
        return;
    }
    ((*pattern).release)
        .expect(
            "non-null function pointer",
        )(&mut (*pattern).base, (*pattern).user_data, surface);
}
#[no_mangle]
pub unsafe extern "C" fn _cairo_raster_source_pattern_init_copy(
    mut abstract_pattern: *mut cairo_pattern_t,
    ...
    memcpy(
        ...
        ::std::mem::size_of::<cairo_raster_source_pattern_t>() as libc::c_ulong,
    );
    status = CAIRO_STATUS_SUCCESS;
    if ((*pattern).copy).is_some() {
        status = ((*pattern).copy)
            .expect(
                "non-null function pointer",
            )(&mut (*pattern).base, (*pattern).user_data, other);
    }
    return status;
}
#[no_mangle]
pub unsafe extern "C" fn _cairo_raster_source_pattern_snapshot(
    mut abstract_pattern: *mut cairo_pattern_t,
) -> cairo_status_t {
    let mut pattern: *mut cairo_raster_source_pattern_t = abstract_pattern
        as *mut cairo_raster_source_pattern_t;
    if ((*pattern).snapshot).is_none() {
        return CAIRO_STATUS_SUCCESS;
    }
    return ((*pattern).snapshot)
        .expect("non-null function pointer")(&mut (*pattern).base, (*pattern).user_data);
}
#[no_mangle]
pub unsafe extern "C" fn _cairo_raster_source_pattern_finish(
    mut abstract_pattern: *mut cairo_pattern_t,
) {
    let mut pattern: *mut cairo_raster_source_pattern_t = abstract_pattern
        as *mut cairo_raster_source_pattern_t;
    if ((*pattern).finish).is_none() {
        return;
    }
    ((*pattern).finish)
        .expect("non-null function pointer")(&mut (*pattern).base, (*pattern).user_data);
...
#[no_mangle]
pub unsafe extern "C" fn cairo_pattern_create_raster_source(
    mut user_data: *mut libc::c_void,
    mut content: cairo_content_t,
    mut width: libc::c_int,
    mut height: libc::c_int,
) -> *mut cairo_pattern_t {
    let mut pattern: *mut cairo_raster_source_pattern_t = 0
        as *mut cairo_raster_source_pattern_t;
    ...
    _cairo_pattern_init(&mut (*pattern).base, CAIRO_PATTERN_TYPE_RASTER_SOURCE);
    (*pattern).base.ref_count.ref_count = 1 as libc::c_int;
    (*pattern).content = content;
    (*pattern).extents.x = 0 as libc::c_int;
    (*pattern).extents.y = 0 as libc::c_int;
    (*pattern).extents.width = width;
    (*pattern).extents.height = height;
    let ref mut fresh0 = (*pattern).user_data;
    *fresh0 = user_data;
    return &mut (*pattern).base;
}
#[no_mangle]
pub unsafe extern "C" fn cairo_raster_source_pattern_set_callback_data(
    mut abstract_pattern: *mut cairo_pattern_t,
    mut data: *mut libc::c_void,
) {
    let mut pattern: *mut cairo_raster_source_pattern_t = 0
        as *mut cairo_raster_source_pattern_t;
    if (*abstract_pattern).type_0 as libc::c_uint
        != CAIRO_PATTERN_TYPE_RASTER_SOURCE as libc::c_int as libc::c_uint
    {
        return;
    }
    pattern = abstract_pattern as *mut cairo_raster_source_pattern_t;
    let ref mut fresh1 = (*pattern).user_data;
    *fresh1 = data;
}
#[no_mangle]
pub unsafe extern "C" fn cairo_raster_source_pattern_get_callback_data(
    mut abstract_pattern: *mut cairo_pattern_t,
) -> *mut libc::c_void {
    let mut pattern: *mut cairo_raster_source_pattern_t = 0
        as *mut cairo_raster_source_pattern_t;
...
#[no_mangle]
pub unsafe extern "C" fn cairo_raster_source_pattern_set_acquire(
    mut abstract_pattern: *mut cairo_pattern_t,
    mut acquire: cairo_raster_source_acquire_func_t,
    mut release: cairo_raster_source_release_func_t,
) {
    let mut pattern: *mut cairo_raster_source_pattern_t = 0
        as *mut cairo_raster_source_pattern_t;
    if (*abstract_pattern).type_0 as libc::c_uint
    ...
    pattern = abstract_pattern as *mut cairo_raster_source_pattern_t;
    let ref mut fresh2 = (*pattern).acquire;
    *fresh2 = acquire;
    let ref mut fresh3 = (*pattern).release;
    *fresh3 = release;
}
#[no_mangle]
pub unsafe extern "C" fn cairo_raster_source_pattern_get_acquire(
    mut abstract_pattern: *mut cairo_pattern_t,
    mut acquire: *mut cairo_raster_source_acquire_func_t,
    mut release: *mut cairo_raster_source_release_func_t,
) {
    let mut pattern: *mut cairo_raster_source_pattern_t = 0
        as *mut cairo_raster_source_pattern_t;
    if (*abstract_pattern).type_0 as libc::c_uint
...
#[no_mangle]
pub unsafe extern "C" fn cairo_raster_source_pattern_set_snapshot(
    mut abstract_pattern: *mut cairo_pattern_t,
    mut snapshot: cairo_raster_source_snapshot_func_t,
) {
    let mut pattern: *mut cairo_raster_source_pattern_t = 0
        as *mut cairo_raster_source_pattern_t;
    if (*abstract_pattern).type_0 as libc::c_uint
    ...
    pattern = abstract_pattern as *mut cairo_raster_source_pattern_t;
    let ref mut fresh4 = (*pattern).snapshot;
    *fresh4 = snapshot;
}
#[no_mangle]
pub unsafe extern "C" fn cairo_raster_source_pattern_get_snapshot(
    mut abstract_pattern: *mut cairo_pattern_t,
...
#[no_mangle]
pub unsafe extern "C" fn cairo_raster_source_pattern_set_copy(
    mut abstract_pattern: *mut cairo_pattern_t,
    mut copy: cairo_raster_source_copy_func_t,
) {
    let mut pattern: *mut cairo_raster_source_pattern_t = 0
        as *mut cairo_raster_source_pattern_t;
    if (*abstract_pattern).type_0 as libc::c_uint
    ...
    pattern = abstract_pattern as *mut cairo_raster_source_pattern_t;
    let ref mut fresh5 = (*pattern).copy;
    *fresh5 = copy;
}
#[no_mangle]
pub unsafe extern "C" fn cairo_raster_source_pattern_get_copy(
    mut abstract_pattern: *mut cairo_pattern_t,
...
#[no_mangle]
pub unsafe extern "C" fn cairo_raster_source_pattern_set_finish(
    mut abstract_pattern: *mut cairo_pattern_t,
    mut finish: cairo_raster_source_finish_func_t,
) {
    let mut pattern: *mut cairo_raster_source_pattern_t = 0
        as *mut cairo_raster_source_pattern_t;
    if (*abstract_pattern).type_0 as libc::c_uint
    ...
    pattern = abstract_pattern as *mut cairo_raster_source_pattern_t;
    let ref mut fresh6 = (*pattern).finish;
    *fresh6 = finish;
}
#[no_mangle]
pub unsafe extern "C" fn cairo_raster_source_pattern_get_finish(
    mut abstract_pattern: *mut cairo_pattern_t,
