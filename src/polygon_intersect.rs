...
pub struct _cairo_bo_start_event {
    ...
    pub point: cairo_bo_intersect_point_t,
...
pub struct _cairo_bo_intersect_ordinate {
    ...
    pub approx: C2RustUnnamed,
}
pub type C2RustUnnamed = libc::c_int;
pub const DEFAULT: C2RustUnnamed = 1;
pub const EXACT: C2RustUnnamed = 0;
pub const EXCESS: C2RustUnnamed = -1;
pub type cairo_bo_event_type_t = libc::c_int;
pub const CAIRO_BO_EVENT_TYPE_START: cairo_bo_event_type_t = 1;
pub const CAIRO_BO_EVENT_TYPE_INTERSECTION: cairo_bo_event_type_t = 0;
...
unsafe extern "C" fn _cairo_freepool_alloc_from_pool(
    ...
    ptr = (*pool).data;
    let ref mut fresh8 = (*pool).data;
    *fresh8 = (*fresh8).offset((*freepool).nodesize as isize);
    let ref mut fresh9 = (*pool).rem;
    *fresh9 = (*fresh9).wrapping_sub((*freepool).nodesize);
...
unsafe extern "C" fn _cairo_freepool_alloc(
    ...
    let ref mut fresh10 = (*freepool).first_free_node;
    *fresh10 = (*node).next;
...
unsafe extern "C" fn _cairo_bo_point32_compare(
    mut a: *const cairo_bo_intersect_point_t,
    mut b: *const cairo_bo_intersect_point_t,
) -> libc::c_int {
    ...
    cmp = (*a).y.approx as libc::c_int - (*b).y.approx as libc::c_int;
    if cmp != 0 {
        return cmp;
    }
...
#[inline]
unsafe extern "C" fn round_to_nearest(
    mut d: cairo_quorem64_t,
    mut den: cairo_int64_t,
) -> cairo_bo_intersect_ordinate_t {
    let mut ordinate: cairo_bo_intersect_ordinate_t = cairo_bo_intersect_ordinate_t {
        ordinate: 0,
        approx: EXACT,
    };
    let mut quo: int32_t = d.quo as int32_t;
    let mut drem_2: cairo_int64_t = d.rem * 2 as libc::c_int as int64_t;
    if drem_2 < -den {
        quo -= 1 as libc::c_int;
        drem_2 = -drem_2;
    } else if !(drem_2 < den) {
    ...
    ordinate
        .approx = (if drem_2 == 0 as libc::c_int as libc::c_long {
        EXACT as libc::c_int
    } else if drem_2 < 0 as libc::c_int as libc::c_long {
        EXCESS as libc::c_int
    } else {
...
unsafe extern "C" fn _cairo_bo_edge_contains_intersect_point(
    ...
    return (_cairo_bo_intersect_ordinate_32_compare((*point).y, (*edge).edge.bottom)
...
unsafe extern "C" fn _cairo_bo_edge_intersect(
    ...
    if intersect_lines(a, b, intersection) == 0 {
        return 0 as libc::c_int;
    }
    if _cairo_bo_edge_contains_intersect_point(a, intersection) == 0 {
        return 0 as libc::c_int;
    }
    if _cairo_bo_edge_contains_intersect_point(b, intersection) == 0 {
        return 0 as libc::c_int;
    }
    return 1 as libc::c_int;
}
...
unsafe extern "C" fn cairo_bo_event_compare(
    ...
    cmp = (*a).type_0 as libc::c_int - (*b).type_0 as libc::c_int;
    if cmp != 0 {
        return cmp;
    }
    return if a < b {
...
unsafe extern "C" fn _cairo_bo_event_queue_insert(
    ...
    mut point: *const cairo_bo_intersect_point_t,
) -> cairo_status_t {
...
unsafe extern "C" fn event_queue_insert_stop(
    mut event_queue: *mut cairo_bo_event_queue_t,
    mut edge: *mut cairo_bo_edge_t,
) -> cairo_status_t {
    let mut point: cairo_bo_intersect_point_t = cairo_bo_intersect_point_t {
    ...
    point
        .x
        .ordinate = _line_compute_intersection_x_for_y(
        &mut (*edge).edge.line,
        point.y.ordinate,
    );
...
#[inline]
unsafe extern "C" fn event_queue_insert_if_intersect_below_current_y(
    mut event_queue: *mut cairo_bo_event_queue_t,
    mut left: *mut cairo_bo_edge_t,
    mut right: *mut cairo_bo_edge_t,
) -> cairo_status_t {
    let mut intersection: cairo_bo_intersect_point_t = cairo_bo_intersect_point_t {
        x: cairo_bo_intersect_ordinate_t {
            ordinate: 0,
            approx: EXACT,
        },
...
unsafe extern "C" fn edges_end(
    mut left: *mut cairo_bo_edge_t,
    mut bot: int32_t,
    mut polygon: *mut cairo_polygon_t,
) {
    let mut l: *mut cairo_bo_deferred_t = &mut (*left).deferred;
    let mut right: *mut cairo_bo_edge_t = (*l).other;
    if ((*right).deferred.other).is_null() {} else {
        __assert_fail(
            b"right->deferred.other == NULL\0" as *const u8 as *const libc::c_char,
            b"../src/cairo-polygon-intersect.c\0" as *const u8 as *const libc::c_char,
            1095 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<
                &[u8; 62],
                &[libc::c_char; 62],
            >(b"void edges_end(cairo_bo_edge_t *, int32_t, cairo_polygon_t *)\0"))
                .as_ptr(),
        );
    }
    if (*l).top < bot {
    ...
    let ref mut fresh59 = (*l).other;
    *fresh59 = 0 as *mut cairo_bo_edge_t;
}
#[inline]
unsafe extern "C" fn edges_start_or_continue(
    mut left: *mut cairo_bo_edge_t,
    mut right: *mut cairo_bo_edge_t,
    mut top: libc::c_int,
    mut polygon: *mut cairo_polygon_t,
) {
    if !right.is_null() {} else {
        __assert_fail(
            ...
            (*::std::mem::transmute::<
                ...
                &[libc::c_char; 91],
            >(
                b"void edges_start_or_continue(cairo_bo_edge_t *, cairo_bo_edge_t *, int, cairo_polygon_t *)\0",
            ))
                .as_ptr(),
        );
    }
    ...
    if !((*left).deferred.other).is_null() {
        if edges_colinear((*left).deferred.other, right) != 0 {
            let mut old: *mut cairo_bo_edge_t = (*left).deferred.other;
            if ((*old).deferred.other).is_null() {} else {
                __assert_fail(
                    b"old->deferred.other == NULL\0" as *const u8 as *const libc::c_char,
                    b"../src/cairo-polygon-intersect.c\0" as *const u8
            ...
            if (*old).edge.line.p2.y > (*old).edge.line.p1.y {} else {
                __assert_fail(
                    b"old->edge.line.p2.y > old->edge.line.p1.y\0" as *const u8
                        as *const libc::c_char,
                    b"../src/cairo-polygon-intersect.c\0" as *const u8
                        as *const libc::c_char,
            ...
            let ref mut fresh60 = (*left).deferred.other;
            *fresh60 = right;
            return;
        }
        edges_end(left, top, polygon);
    }
    if edges_colinear(left, right) == 0 {
        (*left).deferred.top = top;
        let ref mut fresh61 = (*left).deferred.other;
        *fresh61 = right;
    }
}
#[inline]
unsafe extern "C" fn active_edges(
    ...
    let mut winding: [libc::c_int; 2] = [0 as libc::c_int, 0 as libc::c_int];
    while !left.is_null() {
        if winding[0 as libc::c_int as usize] == 0 as libc::c_int
            || winding[1 as libc::c_int as usize] == 0 as libc::c_int
        {} else {
            __assert_fail(
                b"is_zero (winding)\0" as *const u8 as *const libc::c_char,
                b"../src/cairo-polygon-intersect.c\0" as *const u8
                    as *const libc::c_char,
                ...
                (*::std::mem::transmute::<
                    &[u8; 65],
                    &[libc::c_char; 65],
                >(b"void active_edges(cairo_bo_edge_t *, int32_t, cairo_polygon_t *)\0"))
                    .as_ptr(),
            );
        }
        loop {
            winding[(*left).a_or_b as usize] += (*left).edge.dir;
            if !(winding[0 as libc::c_int as usize] == 0 as libc::c_int
                || winding[1 as libc::c_int as usize] == 0 as libc::c_int)
            {
                break;
        ...
        right = (*left).next;
        loop {
            if !((*right).deferred.other).is_null() {
                edges_end(right, top, polygon);
            }
            winding[(*right).a_or_b as usize] += (*right).edge.dir;
            if winding[0 as libc::c_int as usize] == 0 as libc::c_int
                || winding[1 as libc::c_int as usize] == 0 as libc::c_int
            {
...
unsafe extern "C" fn intersection_sweep(
    mut start_events: *mut *mut cairo_bo_event_t,
    mut num_events: libc::c_int,
    mut polygon: *mut cairo_polygon_t,
) -> cairo_status_t {
    ...
    loop {
        ...
        match (*event).type_0 as libc::c_int {
            1 => {
                ...
                status = sweep_line_insert(&mut sweep_line, e1);
                if status as u64 != 0 {
                    break;
                }
                status = event_queue_insert_stop(&mut event_queue, e1);
                if status as u64 != 0 {
                    break;
                }
                left = (*e1).prev;
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_polygon_intersect(
    mut a: *mut cairo_polygon_t,
    mut winding_a: libc::c_int,
    mut b: *mut cairo_polygon_t,
    mut winding_b: libc::c_int,
) -> cairo_status_t {
    let mut status: cairo_status_t = CAIRO_STATUS_SUCCESS;
    let mut stack_events: [cairo_bo_start_event_t; 23] = [cairo_bo_start_event_t {
        type_0: CAIRO_BO_EVENT_TYPE_INTERSECTION,
        point: cairo_bo_intersect_point_t {
            x: cairo_bo_intersect_ordinate_t {
                ordinate: 0,
                approx: EXACT,
            },
        ...
        edge: cairo_bo_edge_t {
            ...
            deferred: cairo_bo_deferred_t {
                other: 0 as *mut cairo_bo_edge_t,
                top: 0,
            },
        },
    }; 23];
    let mut events: *mut cairo_bo_start_event_t = 0 as *mut cairo_bo_start_event_t;
    let mut stack_event_ptrs: [*mut cairo_bo_event_t; 24] = [0
        as *mut cairo_bo_event_t; 24];
    let mut event_ptrs: *mut *mut cairo_bo_event_t = 0 as *mut *mut cairo_bo_event_t;
    let mut num_events: libc::c_int = 0;
    let mut i: libc::c_int = 0;
    let mut j: libc::c_int = 0;
    if winding_a != CAIRO_FILL_RULE_WINDING as libc::c_int {
        status = _cairo_polygon_reduce(a, winding_a as cairo_fill_rule_t);
        if status as u64 != 0 {
            return status;
        }
    }
    if winding_b != CAIRO_FILL_RULE_WINDING as libc::c_int {
        status = _cairo_polygon_reduce(b, winding_b as cairo_fill_rule_t);
        if status as u64 != 0 {
            return status;
        }
    }
    if 0 as libc::c_int == (*a).num_edges {
        return CAIRO_STATUS_SUCCESS;
    }
    if 0 as libc::c_int == (*b).num_edges {
        (*a).num_edges = 0 as libc::c_int;
        return CAIRO_STATUS_SUCCESS;
    }
    ...
    if num_events
        > (::std::mem::size_of::<[cairo_bo_start_event_t; 23]>() as libc::c_ulong)
            .wrapping_div(
    ...
    i = 0 as libc::c_int;
    while i < (*a).num_edges {
        let ref mut fresh62 = *event_ptrs.offset(j as isize);
        *fresh62 = &mut *events.offset(j as isize) as *mut cairo_bo_start_event_t
            as *mut cairo_bo_event_t;
        (*events.offset(j as isize)).type_0 = CAIRO_BO_EVENT_TYPE_START;
        (*events.offset(j as isize))
            .point
            .y
            .ordinate = (*((*a).edges).offset(i as isize)).top;
        (*events.offset(j as isize)).point.y.approx = EXACT;
        (*events.offset(j as isize))
            .point
            .x
            .ordinate = _line_compute_intersection_x_for_y(
            &mut (*((*a).edges).offset(i as isize)).line,
            (*events.offset(j as isize)).point.y.ordinate,
        );
        (*events.offset(j as isize)).point.x.approx = EXACT;
        (*events.offset(j as isize)).edge.a_or_b = 0 as libc::c_int;
        (*events.offset(j as isize)).edge.edge = *((*a).edges).offset(i as isize);
        let ref mut fresh63 = (*events.offset(j as isize)).edge.deferred.other;
        *fresh63 = 0 as *mut cairo_bo_edge_t;
        let ref mut fresh64 = (*events.offset(j as isize)).edge.prev;
        *fresh64 = 0 as *mut cairo_bo_edge_t;
        let ref mut fresh65 = (*events.offset(j as isize)).edge.next;
    ...
    i = 0 as libc::c_int;
    while i < (*b).num_edges {
        let ref mut fresh66 = *event_ptrs.offset(j as isize);
        *fresh66 = &mut *events.offset(j as isize) as *mut cairo_bo_start_event_t
            as *mut cairo_bo_event_t;
        (*events.offset(j as isize)).type_0 = CAIRO_BO_EVENT_TYPE_START;
        (*events.offset(j as isize))
            .point
            .y
            .ordinate = (*((*b).edges).offset(i as isize)).top;
        (*events.offset(j as isize)).point.y.approx = EXACT;
        (*events.offset(j as isize))
            .point
            .x
            .ordinate = _line_compute_intersection_x_for_y(
            &mut (*((*b).edges).offset(i as isize)).line,
            (*events.offset(j as isize)).point.y.ordinate,
        );
        (*events.offset(j as isize)).point.x.approx = EXACT;
        (*events.offset(j as isize)).edge.a_or_b = 1 as libc::c_int;
        (*events.offset(j as isize)).edge.edge = *((*b).edges).offset(i as isize);
        let ref mut fresh67 = (*events.offset(j as isize)).edge.deferred.other;
        *fresh67 = 0 as *mut cairo_bo_edge_t;
        let ref mut fresh68 = (*events.offset(j as isize)).edge.prev;
        *fresh68 = 0 as *mut cairo_bo_edge_t;
        let ref mut fresh69 = (*events.offset(j as isize)).edge.next;
        *fresh69 = 0 as *mut cairo_bo_edge_t;
        j += 1;
        i += 1;
    }
    if j == num_events {} else {
        __assert_fail(
            b"j == num_events\0" as *const u8 as *const libc::c_char,
            b"../src/cairo-polygon-intersect.c\0" as *const u8 as *const libc::c_char,
            ...
            (*::std::mem::transmute::<
                ...
                &[libc::c_char; 88],
            >(
                b"cairo_status_t _cairo_polygon_intersect(cairo_polygon_t *, int, cairo_polygon_t *, int)\0",
            ))
                .as_ptr(),
        );
    }
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_polygon_intersect_with_boxes(
    mut polygon: *mut cairo_polygon_t,
    mut winding: *mut cairo_fill_rule_t,
    mut boxes: *mut cairo_box_t,
    mut num_boxes: libc::c_int,
) -> cairo_status_t {
    let mut b: cairo_polygon_t = cairo_polygon_t {
    ...
    if num_boxes == 0 as libc::c_int {
        (*polygon).num_edges = 0 as libc::c_int;
        return CAIRO_STATUS_SUCCESS;
    }
    n = 0 as libc::c_int;
    while n < num_boxes {
        if (*polygon).extents.p1.x >= (*boxes.offset(n as isize)).p1.x
            && (*polygon).extents.p2.x <= (*boxes.offset(n as isize)).p2.x
            && (*polygon).extents.p1.y >= (*boxes.offset(n as isize)).p1.y
            && (*polygon).extents.p2.y <= (*boxes.offset(n as isize)).p2.y
        {
            return CAIRO_STATUS_SUCCESS;
        }
        n += 1;
    }
    _cairo_polygon_init(&mut b, 0 as *const cairo_box_t, 0 as libc::c_int);
    n = 0 as libc::c_int;
    while n < num_boxes {
        if (*boxes.offset(n as isize)).p2.x > (*polygon).extents.p1.x
            && (*boxes.offset(n as isize)).p1.x < (*polygon).extents.p2.x
            && (*boxes.offset(n as isize)).p2.y > (*polygon).extents.p1.y
            && (*boxes.offset(n as isize)).p1.y < (*polygon).extents.p2.y
        {
            let mut p1: cairo_point_t = cairo_point_t { x: 0, y: 0 };
            let mut p2: cairo_point_t = cairo_point_t { x: 0, y: 0 };
            p1.y = (*boxes.offset(n as isize)).p1.y;
            p2.y = (*boxes.offset(n as isize)).p2.y;
            p1.x = (*boxes.offset(n as isize)).p1.x;
            p2.x = p1.x;
            _cairo_polygon_add_external_edge(
                &mut b as *mut cairo_polygon_t as *mut libc::c_void,
                &mut p1,
                &mut p2,
            );
            p1.x = (*boxes.offset(n as isize)).p2.x;
            p2.x = p1.x;
            _cairo_polygon_add_external_edge(
                &mut b as *mut cairo_polygon_t as *mut libc::c_void,
                &mut p2,
    ...
    status = _cairo_polygon_intersect(
        polygon,
        *winding as libc::c_int,
        &mut b,
        CAIRO_FILL_RULE_WINDING as libc::c_int,
    );
    _cairo_polygon_fini(&mut b);
