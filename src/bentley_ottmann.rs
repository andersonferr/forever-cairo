...
pub struct _cairo_bo_edge {
    ...
    pub colinear: *mut cairo_bo_edge_t,
...
unsafe extern "C" fn _cairo_realloc_ab(
    ...
    let mut c: size_t = 0;
    let (fresh6, fresh7) = a.overflowing_mul(size);
    *(&mut c as *mut size_t) = fresh6;
    if fresh7 {
        return 0 as *mut libc::c_void;
    }
...
unsafe extern "C" fn _cairo_bo_sweep_line_compare_edges(
    mut sweep_line: *const cairo_bo_sweep_line_t,
    mut a: *const cairo_bo_edge_t,
    mut b: *const cairo_bo_edge_t,
) -> libc::c_int {
    let mut cmp: libc::c_int = 0;
    cmp = _cairo_lines_compare_at_y(
        &(*a).edge.line,
        &(*b).edge.line,
...
unsafe extern "C" fn _cairo_bo_event_queue_init(
    ...
    let ref mut fresh28 = (*event_queue).start_events;
    *fresh28 = start_events;
    ...
    let ref mut fresh29 = *((*event_queue).pqueue.elements)
        .offset(1 as libc::c_int as isize);
    *fresh29 = 0 as *mut cairo_bo_event_t;
}
...
unsafe extern "C" fn _cairo_bo_event_queue_insert_if_intersect_below_current_y(
    ...
    if (if (*left).edge.line.p1.x > (*left).edge.line.p2.x {
        (*left).edge.line.p1.x
    } else {
        (*left).edge.line.p2.x
    })
        <= (if (*right).edge.line.p1.x < (*right).edge.line.p2.x {
            (*right).edge.line.p1.x
        } else {
            (*right).edge.line.p2.x
        })
    {
        return CAIRO_STATUS_SUCCESS;
    }
    if cairo_lines_equal(&mut (*left).edge.line, &mut (*right).edge.line) != 0 {
        return CAIRO_STATUS_SUCCESS;
    }
...
unsafe extern "C" fn _cairo_bo_sweep_line_init(
    ...
    let ref mut fresh30 = (*sweep_line).head;
    *fresh30 = 0 as *mut cairo_bo_edge_t;
    let ref mut fresh31 = (*sweep_line).stopped;
...
unsafe extern "C" fn _cairo_bo_sweep_line_insert(
    ...
    if !((*sweep_line).current_edge).is_null() {
        ...
        *fresh46 = edge;
        let ref mut fresh47 = (*edge).next;
        *fresh47 = 0 as *mut cairo_bo_edge_t;
    }
    let ref mut fresh48 = (*sweep_line).current_edge;
    *fresh48 = edge;
}
unsafe extern "C" fn _cairo_bo_sweep_line_delete(
    ...
    if !((*edge).prev).is_null() {
        let ref mut fresh49 = (*(*edge).prev).next;
        *fresh49 = (*edge).next;
    } else {
        let ref mut fresh50 = (*sweep_line).head;
        *fresh50 = (*edge).next;
    }
    if !((*edge).next).is_null() {
        let ref mut fresh51 = (*(*edge).next).prev;
        *fresh51 = (*edge).prev;
    }
    if (*sweep_line).current_edge == edge {
        let ref mut fresh52 = (*sweep_line).current_edge;
        *fresh52 = if !((*edge).prev).is_null() { (*edge).prev } else { (*edge).next };
    }
}
unsafe extern "C" fn _cairo_bo_sweep_line_swap(
    ...
    if !((*left).prev).is_null() {
        let ref mut fresh53 = (*(*left).prev).next;
        *fresh53 = right;
    } else {
        let ref mut fresh54 = (*sweep_line).head;
        *fresh54 = right;
    }
    if !((*right).next).is_null() {
        let ref mut fresh55 = (*(*right).next).prev;
        *fresh55 = left;
    }
    let ref mut fresh56 = (*left).next;
    *fresh56 = (*right).next;
    let ref mut fresh57 = (*right).next;
    *fresh57 = left;
    let ref mut fresh58 = (*right).prev;
    *fresh58 = (*left).prev;
    let ref mut fresh59 = (*left).prev;
    *fresh59 = right;
}
#[inline]
unsafe extern "C" fn edges_colinear(
    ...
    if ((*a).colinear as uintptr_t & !(1 as libc::c_int) as libc::c_ulong)
        as *mut cairo_bo_edge_t == b as *mut cairo_bo_edge_t
    {
        return ((*a).colinear as uintptr_t & 1 as libc::c_int as libc::c_ulong)
            as cairo_bool_t;
    }
    if ((*b).colinear as uintptr_t & !(1 as libc::c_int) as libc::c_ulong)
        as *mut cairo_bo_edge_t == a
    {
        p = ((*b).colinear as uintptr_t & 1 as libc::c_int as libc::c_ulong)
            as libc::c_uint;
        let ref mut fresh60 = (*a).colinear;
        *fresh60 = (b as uintptr_t | p as libc::c_ulong) as *mut cairo_bo_edge_t;
        return p as cairo_bool_t;
    }
    p = 0 as libc::c_int as libc::c_uint;
    p
        |= ((((*a).edge.line.p1.x == (*b).edge.line.p1.x) as libc::c_int)
            << 0 as libc::c_int) as libc::c_uint;
    p
        |= ((((*a).edge.line.p1.y == (*b).edge.line.p1.y) as libc::c_int)
            << 1 as libc::c_int) as libc::c_uint;
    p
        |= ((((*a).edge.line.p2.x == (*b).edge.line.p2.x) as libc::c_int)
            << 3 as libc::c_int) as libc::c_uint;
    p
        |= ((((*a).edge.line.p2.y == (*b).edge.line.p2.y) as libc::c_int)
    ...
    {
        let ref mut fresh61 = (*a).colinear;
        *fresh61 = (b as uintptr_t | 1 as libc::c_int as libc::c_ulong)
            as *mut cairo_bo_edge_t;
        return 1 as libc::c_int;
    }
    if _slope_compare(a, b) != 0 {
        let ref mut fresh62 = (*a).colinear;
        *fresh62 = (b as uintptr_t | 0 as libc::c_int as libc::c_ulong)
            as *mut cairo_bo_edge_t;
        return 0 as libc::c_int;
    }
    if p != 0 as libc::c_int as libc::c_uint {
        p = (p >> 1 as libc::c_int & p & 5 as libc::c_int as libc::c_uint
            != 0 as libc::c_int as libc::c_uint) as libc::c_int as libc::c_uint;
    } else if (*a).edge.line.p1.y < (*b).edge.line.p1.y {
        p = (edge_compare_for_y_against_x(b, (*a).edge.line.p1.y, (*a).edge.line.p1.x)
            == 0 as libc::c_int) as libc::c_int as libc::c_uint;
    } else {
        p = (edge_compare_for_y_against_x(a, (*b).edge.line.p1.y, (*b).edge.line.p1.x)
            == 0 as libc::c_int) as libc::c_int as libc::c_uint;
    }
    let ref mut fresh63 = (*a).colinear;
    *fresh63 = (b as uintptr_t | p as libc::c_ulong) as *mut cairo_bo_edge_t;
    return p as cairo_bool_t;
}
unsafe extern "C" fn _cairo_bo_edge_end_trap(
    ...
    let ref mut fresh64 = (*trap).right;
...
unsafe extern "C" fn _cairo_bo_edge_start_or_continue_trap(
    ...
    if !right.is_null() {} else {
        __assert_fail(
            ...
            b"../src/cairo-bentley-ottmann.c\0" as *const u8 as *const libc::c_char,
            1197 as libc::c_int as libc::c_uint,
            (*::std::mem::transmute::<
                &[u8; 103],
                &[libc::c_char; 103],
            >(
                b"void _cairo_bo_edge_start_or_continue_trap(cairo_bo_edge_t *, cairo_bo_edge_t *, int, cairo_traps_t *)\0",
            ))
                .as_ptr(),
        );
    }
    if !((*left).deferred_trap.right).is_null() {
        if edges_colinear((*left).deferred_trap.right, right) != 0 {
            let ref mut fresh65 = (*left).deferred_trap.right;
            *fresh65 = right;
            return;
        }
    ...
    if edges_colinear(left, right) == 0 {
        ...
        let ref mut fresh66 = (*left).deferred_trap.right;
        *fresh66 = right;
    }
}
#[inline]
unsafe extern "C" fn _active_edges_to_traps(
    mut pos: *mut cairo_bo_edge_t,
    mut top: int32_t,
    ...
    in_out = 0 as libc::c_int;
    left = pos;
    while !pos.is_null() {
        if pos != left && !((*pos).deferred_trap.right).is_null() {
            if ((*left).deferred_trap.right).is_null() && edges_colinear(left, pos) != 0
            {
                (*left).deferred_trap = (*pos).deferred_trap;
                let ref mut fresh67 = (*pos).deferred_trap.right;
        ...
        if in_out as libc::c_uint & mask == 0 as libc::c_int as libc::c_uint {
            if ((*pos).next).is_null() || edges_colinear(pos, (*pos).next) == 0 {
...
unsafe extern "C" fn _cairo_bentley_ottmann_tessellate_bo_edges(
    mut start_events: *mut *mut cairo_bo_event_t,
    mut num_events: libc::c_int,
    mut fill_rule: libc::c_uint,
    mut traps: *mut cairo_traps_t,
    mut num_intersections: *mut libc::c_int,
) -> cairo_status_t {
    let mut current_block: u64;
    let mut status: cairo_status_t = CAIRO_STATUS_SUCCESS;
    let mut intersection_count: libc::c_int = 0 as libc::c_int;
    ...
    if fill_rule == CAIRO_FILL_RULE_WINDING as libc::c_int as libc::c_uint {
        fill_rule = -(1 as libc::c_int) as libc::c_uint;
    } else {
        fill_rule = 1 as libc::c_int as libc::c_uint;
    }
    ...
    loop {
        ...
        if (*event).point.y != sweep_line.current_y {
            e1 = sweep_line.stopped;
            while !e1.is_null() {
                if !((*e1).deferred_trap.right).is_null() {
                    _cairo_bo_edge_end_trap(e1, (*e1).edge.bottom, traps);
                }
                e1 = (*e1).next;
            }
            sweep_line.stopped = 0 as *mut cairo_bo_edge_t;
        ...
        match (*event).type_0 as libc::c_uint {
            2 => {
                ...
                _cairo_bo_sweep_line_insert(&mut sweep_line, e1);
                status = _cairo_bo_event_queue_insert_stop(&mut event_queue, e1);
                if status as u64 != 0 {
                    current_block = 4924794348842672891;
                    break;
                }
                left = sweep_line.stopped;
                while !left.is_null() {
                    if (*e1).edge.top <= (*left).edge.bottom
                        && edges_colinear(e1, left) != 0
                    {
                        (*e1).deferred_trap = (*left).deferred_trap;
                        if !((*left).prev).is_null() {
                            let ref mut fresh68 = (*left).prev;
                            *fresh68 = (*left).next;
                        } else {
                            sweep_line.stopped = (*left).next;
                        }
                        if !((*left).next).is_null() {
                            let ref mut fresh69 = (*(*left).next).prev;
                            *fresh69 = (*left).prev;
                        }
                        break;
                    } else {
            ...
            0 => {
                ...
                if !((*e1).deferred_trap.right).is_null() {
                    let ref mut fresh70 = (*e1).next;
                    *fresh70 = sweep_line.stopped;
                    if !(sweep_line.stopped).is_null() {
                        let ref mut fresh71 = (*sweep_line.stopped).prev;
                        *fresh71 = e1;
                    }
                    sweep_line.stopped = e1;
                    let ref mut fresh72 = (*e1).prev;
                    *fresh72 = 0 as *mut cairo_bo_edge_t;
                }
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_bentley_ottmann_tessellate_polygon(
    mut traps: *mut cairo_traps_t,
    mut polygon: *const cairo_polygon_t,
    mut fill_rule: cairo_fill_rule_t,
) -> cairo_status_t {
    let mut intersections: libc::c_int = 0;
    let mut stack_events: [cairo_bo_start_event_t; 23] = [cairo_bo_start_event_t {
        ...
        edge: cairo_bo_edge_t {
            ...
            colinear: 0 as *mut cairo_bo_edge_t,
    ...
    let mut stack_event_y: [*mut cairo_bo_start_event_t; 64] = [0
        as *mut cairo_bo_start_event_t; 64];
    let mut event_y: *mut *mut cairo_bo_start_event_t = 0
        as *mut *mut cairo_bo_start_event_t;
    let mut i: libc::c_int = 0;
    ...
    if (*polygon).num_limits != 0 {
        ...
        if ymax > 64 as libc::c_int {
            event_y = _cairo_malloc_ab(
                ::std::mem::size_of::<*mut cairo_bo_event_t>() as libc::c_ulong,
                ymax as size_t,
            ) as *mut *mut cairo_bo_start_event_t;
            if event_y.is_null() {
            ...
            event_y = stack_event_y.as_mut_ptr();
        }
        memset(
            event_y as *mut libc::c_void,
            0 as libc::c_int,
            (ymax as libc::c_ulong)
                .wrapping_mul(
                    ::std::mem::size_of::<*mut cairo_bo_event_t>() as libc::c_ulong,
                ),
        );
            ...
            ) as libc::c_int
    {
        ...
        if events.is_null() {
            if event_y != stack_event_y.as_mut_ptr() {
                free(event_y as *mut libc::c_void);
            }
            return _cairo_error(CAIRO_STATUS_NO_MEMORY);
        }
    ...
    while i < num_events {
        ...
        let ref mut fresh73 = (*events.offset(i as isize)).edge.deferred_trap.right;
        *fresh73 = 0 as *mut cairo_bo_edge_t;
        let ref mut fresh74 = (*events.offset(i as isize)).edge.prev;
        *fresh74 = 0 as *mut cairo_bo_edge_t;
        let ref mut fresh75 = (*events.offset(i as isize)).edge.next;
        *fresh75 = 0 as *mut cairo_bo_edge_t;
        let ref mut fresh76 = (*events.offset(i as isize)).edge.colinear;
        *fresh76 = 0 as *mut cairo_bo_edge_t;
        if !event_y.is_null() {
            y = _cairo_fixed_integer_floor((*events.offset(i as isize)).point.y) - ymin;
            let ref mut fresh77 = (*events.offset(i as isize)).edge.next;
            *fresh77 = *event_y.offset(y as isize) as *mut cairo_bo_edge_t;
            let ref mut fresh78 = *event_y.offset(y as isize);
            *fresh78 = &mut *events.offset(i as isize) as *mut cairo_bo_start_event_t;
        } else {
            let ref mut fresh79 = *event_ptrs.offset(i as isize);
            *fresh79 = &mut *events.offset(i as isize) as *mut cairo_bo_start_event_t
    ...
    if !event_y.is_null() {
        i = 0 as libc::c_int;
        y = i;
        while y < ymax && i < num_events {
            let mut e: *mut cairo_bo_start_event_t = 0 as *mut cairo_bo_start_event_t;
            let mut j: libc::c_int = i;
            e = *event_y.offset(y as isize);
            while !e.is_null() {
                let fresh80 = i;
                i = i + 1;
                let ref mut fresh81 = *event_ptrs.offset(fresh80 as isize);
                *fresh81 = e as *mut cairo_bo_event_t;
                e = (*e).edge.next as *mut cairo_bo_start_event_t;
            }
            if i > j + 1 as libc::c_int {
                _cairo_bo_event_queue_sort(
                    event_ptrs.offset(j as isize),
                    (i - j) as libc::c_uint,
                );
            }
            y += 1;
        ...
        _cairo_bo_event_queue_sort(event_ptrs, i as libc::c_uint);
    }
    let ref mut fresh82 = *event_ptrs.offset(i as isize);
    *fresh82 = 0 as *mut cairo_bo_event_t;
    status = _cairo_bentley_ottmann_tessellate_bo_edges(
        event_ptrs,
        num_events,
        fill_rule as libc::c_uint,
        traps,
        &mut intersections,
    );
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_bentley_ottmann_tessellate_traps(
    mut traps: *mut cairo_traps_t,
    mut fill_rule: cairo_fill_rule_t,
) -> cairo_status_t {
    let mut current_block: u64;
