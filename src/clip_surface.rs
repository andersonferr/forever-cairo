...
#[no_mangle]
pub unsafe extern "C" fn _cairo_clip_combine_with_surface(
    mut clip: *const cairo_clip_t,
    mut dst: *mut cairo_surface_t,
    mut dst_x: libc::c_int,
    mut dst_y: libc::c_int,
) -> cairo_status_t {
    let mut copy_path: *mut cairo_clip_path_t = 0 as *mut cairo_clip_path_t;
    ...
    copy_path = (*copy).path;
    let ref mut fresh0 = (*copy).path;
    *fresh0 = 0 as *mut cairo_clip_path_t;
    ...
    if _cairo_clip_is_region(copy) != 0 {
        clip = copy;
    }
    clip_path = copy_path;
    while status as libc::c_uint == CAIRO_STATUS_SUCCESS as libc::c_int as libc::c_uint
        && !clip_path.is_null()
    {
...
unsafe extern "C" fn _cairo_path_fixed_add_box(
    ...
    mut fx: cairo_fixed_t,
    mut fy: cairo_fixed_t,
) -> cairo_status_t {
    let mut status: cairo_status_t = CAIRO_STATUS_SUCCESS;
    status = _cairo_path_fixed_move_to(path, (*box_0).p1.x + fx, (*box_0).p1.y + fy);
    if status as u64 != 0 {
        return status;
    }
    status = _cairo_path_fixed_line_to(path, (*box_0).p2.x + fx, (*box_0).p1.y + fy);
    if status as u64 != 0 {
        return status;
    }
    status = _cairo_path_fixed_line_to(path, (*box_0).p2.x + fx, (*box_0).p2.y + fy);
    if status as u64 != 0 {
        return status;
    }
    status = _cairo_path_fixed_line_to(path, (*box_0).p1.x + fx, (*box_0).p2.y + fy);
    if status as u64 != 0 {
        return status;
    }
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_clip_get_surface(
    mut clip: *const cairo_clip_t,
    ...
    let mut status: cairo_status_t = CAIRO_STATUS_SUCCESS;
    let mut copy: *mut cairo_clip_t = 0 as *mut cairo_clip_t;
    let mut region: *mut cairo_clip_t = 0 as *mut cairo_clip_t;
    let mut copy_path: *mut cairo_clip_path_t = 0 as *mut cairo_clip_path_t;
    let mut clip_path: *mut cairo_clip_path_t = 0 as *mut cairo_clip_path_t;
    if (*clip).num_boxes != 0 {
    ...
    copy = _cairo_clip_copy_with_translation(
        clip,
        -(*clip).extents.x,
        -(*clip).extents.y,
    );
    copy_path = (*copy).path;
    let ref mut fresh2 = (*copy).path;
    *fresh2 = 0 as *mut cairo_clip_path_t;
    region = copy;
    if _cairo_clip_is_region(copy) == 0 {
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_clip_get_image(
    mut clip: *const cairo_clip_t,
    mut target: *mut cairo_surface_t,
