...
extern "C" {
    ...
    fn _cairo_clip_copy_with_translation(
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_surface_offset_paint(
    ...
    let mut dev_clip: *mut cairo_clip_t = clip as *mut cairo_clip_t;
    ...
    if x | y != 0 {
        ...
        cairo_matrix_init_translate(&mut m, x as libc::c_double, y as libc::c_double);
    ...
    if dev_clip != clip as *mut cairo_clip_t {
        _cairo_clip_destroy(dev_clip);
    }
    return status;
}
#[no_mangle]
pub unsafe extern "C" fn _cairo_surface_offset_mask(
    mut target: *mut cairo_surface_t,
    mut x: libc::c_int,
    mut y: libc::c_int,
    mut op: cairo_operator_t,
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_surface_offset_stroke(
    ...
    if x | y != 0 {
        ...
        if status as u64 != 0 {
            current_block = 15804038890190546029;
        } else {
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_surface_offset_fill(
    mut surface: *mut cairo_surface_t,
    mut x: libc::c_int,
    mut y: libc::c_int,
    mut op: cairo_operator_t,
    ...
    if x | y != 0 {
        ...
        status = _cairo_path_fixed_init_copy(&mut path_copy, dev_path);
        if status as u64 != 0 {
            current_block = 10221634434118798177;
        } else {
            _cairo_path_fixed_translate(
                &mut path_copy,
                _cairo_fixed_from_int(-x),
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_surface_offset_glyphs(
    mut surface: *mut cairo_surface_t,
    mut x: libc::c_int,
    mut y: libc::c_int,
    mut op: cairo_operator_t,
    ...
    if x | y != 0 {
        ...
        i = 0 as libc::c_int;
        while i < num_glyphs {
            (*dev_glyphs.offset(i as isize)).x -= x as libc::c_double;
            (*dev_glyphs.offset(i as isize)).y -= y as libc::c_double;
            i += 1;
        }
    }
