...
extern "C" {
    ...
    fn _cairo_clip_reduce_for_composite(
        clip: *const cairo_clip_t,
    ...
    fn _cairo_operator_bounded_by_either(op: cairo_operator_t) -> uint32_t;
    fn _cairo_path_fixed_approximate_fill_extents(
        path: *const cairo_path_fixed_t,
        extents: *mut cairo_rectangle_int_t,
    );
    fn _cairo_path_fixed_approximate_stroke_extents(
        path: *const cairo_path_fixed_t,
    ...
    fn _cairo_matrix_is_pixman_translation(
    ...
    fn _cairo_pattern_sampled_area(
        pattern: *const cairo_pattern_t,
        extents: *const cairo_rectangle_int_t,
        sample: *mut cairo_rectangle_int_t,
    );
    ...
    fn _cairo_pattern_analyze_filter(pattern: *const cairo_pattern_t) -> cairo_filter_t;
}
...
pub const CAIRO_OPERATOR_BOUND_BY_SOURCE: C2RustUnnamed = 4;
pub const CAIRO_OPERATOR_BOUND_BY_MASK: C2RustUnnamed = 2;
#[inline]
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_composite_rectangles_fini(
    mut extents: *mut cairo_composite_rectangles_t,
) {
    _cairo_clip_destroy((*extents).clip);
    let ref mut fresh0 = (*extents).clip;
    *fresh0 = 0 as *mut cairo_clip_t;
}
unsafe extern "C" fn _cairo_composite_reduce_pattern(
    mut src: *const cairo_pattern_t,
    mut dst: *mut cairo_pattern_union_t,
) {
    let mut tx: libc::c_int = 0;
    let mut ty: libc::c_int = 0;
    _cairo_pattern_init_static_copy(&mut (*dst).base, src);
    if (*dst).base.type_0 as libc::c_uint
        == CAIRO_PATTERN_TYPE_SOLID as libc::c_int as libc::c_uint
    {
        return;
    }
    (*dst).base.filter = _cairo_pattern_analyze_filter(&mut (*dst).base);
    ty = 0 as libc::c_int;
    tx = ty;
    if _cairo_matrix_is_pixman_translation(
        &mut (*dst).base.matrix,
        (*dst).base.filter,
        &mut tx,
        &mut ty,
    ...
    {
        (*dst).base.matrix.x0 = tx as libc::c_double;
        (*dst).base.matrix.y0 = ty as libc::c_double;
    }
}
#[inline]
unsafe extern "C" fn _cairo_composite_rectangles_init(
    mut extents: *mut cairo_composite_rectangles_t,
    ...
    let ref mut fresh1 = (*extents).clip;
    *fresh1 = 0 as *mut cairo_clip_t;
    if _cairo_clip_is_all_clipped(clip) != 0 {
        return 0 as libc::c_int;
    }
    let ref mut fresh2 = (*extents).surface;
    *fresh2 = surface;
    (*extents).op = op;
    _cairo_surface_get_extents(surface, &mut (*extents).destination);
    ...
    let ref mut fresh3 = (*extents).original_source_pattern;
    *fresh3 = source;
    _cairo_composite_reduce_pattern(source, &mut (*extents).source_pattern);
    ...
    if (*extents).is_bounded
        & CAIRO_OPERATOR_BOUND_BY_SOURCE as libc::c_int as libc::c_uint != 0
    {
        if _cairo_rectangle_intersect(&mut (*extents).bounded, &mut (*extents).source)
            == 0
        {
            return 0 as libc::c_int;
        }
    }
    let ref mut fresh4 = (*extents).original_mask_pattern;
    *fresh4 = 0 as *const cairo_pattern_t;
    (*extents).mask_pattern.base.type_0 = CAIRO_PATTERN_TYPE_SOLID;
    (*extents).mask_pattern.solid.color.alpha = 1.0f64;
    (*extents)
        .mask_pattern
        .solid
        .color
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_composite_rectangles_init_for_paint(
    mut extents: *mut cairo_composite_rectangles_t,
    mut surface: *mut cairo_surface_t,
    mut op: cairo_operator_t,
    mut source: *const cairo_pattern_t,
    ...
    if !(_cairo_composite_rectangles_init(extents, surface, op, source, clip) == 0) {
        (*extents).mask = (*extents).destination;
        let ref mut fresh5 = (*extents).clip;
        *fresh5 = _cairo_clip_reduce_for_composite(clip, extents);
        if !(_cairo_clip_is_all_clipped((*extents).clip) != 0) {
...
unsafe extern "C" fn _cairo_composite_rectangles_intersect(
    mut extents: *mut cairo_composite_rectangles_t,
    mut clip: *const cairo_clip_t,
) -> cairo_int_status_t {
    if _cairo_rectangle_intersect(&mut (*extents).bounded, &mut (*extents).mask) == 0
        && (*extents).is_bounded
            & CAIRO_OPERATOR_BOUND_BY_MASK as libc::c_int as libc::c_uint != 0
    {
        return CAIRO_INT_STATUS_NOTHING_TO_DO;
    }
    if (*extents).is_bounded
        == (CAIRO_OPERATOR_BOUND_BY_MASK as libc::c_int
            | CAIRO_OPERATOR_BOUND_BY_SOURCE as libc::c_int) as libc::c_uint
    {
        (*extents).unbounded = (*extents).bounded;
    } else if (*extents).is_bounded
        & CAIRO_OPERATOR_BOUND_BY_MASK as libc::c_int as libc::c_uint != 0
    {
        if _cairo_rectangle_intersect(&mut (*extents).unbounded, &mut (*extents).mask)
            == 0
        {
            return CAIRO_INT_STATUS_NOTHING_TO_DO;
        }
    ...
    *fresh6 = _cairo_clip_reduce_for_composite(clip, extents);
    if _cairo_clip_is_all_clipped((*extents).clip) != 0 {
        return CAIRO_INT_STATUS_NOTHING_TO_DO;
    }
    if _cairo_rectangle_intersect(
        ...
        != CAIRO_PATTERN_TYPE_SOLID as libc::c_int as libc::c_uint
    {
        ...
        if (*extents).mask_sample_area.width == 0 as libc::c_int
            || (*extents).mask_sample_area.height == 0 as libc::c_int
        {
            _cairo_composite_rectangles_fini(extents);
            return CAIRO_INT_STATUS_NOTHING_TO_DO;
        }
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_composite_rectangles_intersect_source_extents(
    mut extents: *mut cairo_composite_rectangles_t,
    ...
    _cairo_rectangle_intersect(&mut (*extents).source, &mut rect);
    rect = (*extents).bounded;
    if _cairo_rectangle_intersect(&mut (*extents).bounded, &mut (*extents).source) == 0
        && (*extents).is_bounded
            & CAIRO_OPERATOR_BOUND_BY_SOURCE as libc::c_int as libc::c_uint != 0
    {
        return CAIRO_INT_STATUS_NOTHING_TO_DO;
    ...
    *fresh7 = _cairo_clip_reduce_for_composite(clip, extents);
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_composite_rectangles_intersect_mask_extents(
    mut extents: *mut cairo_composite_rectangles_t,
    mut box_0: *const cairo_box_t,
) -> cairo_int_status_t {
    ...
    *fresh8 = _cairo_clip_reduce_for_composite(clip, extents);
    if clip != (*extents).clip {
        _cairo_clip_destroy(clip);
    }
    if _cairo_clip_is_all_clipped((*extents).clip) != 0 {
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_composite_rectangles_init_for_mask(
    mut extents: *mut cairo_composite_rectangles_t,
    mut surface: *mut cairo_surface_t,
    mut op: cairo_operator_t,
    mut source: *const cairo_pattern_t,
    ...
    let ref mut fresh9 = (*extents).original_mask_pattern;
    *fresh9 = mask;
    _cairo_composite_reduce_pattern(mask, &mut (*extents).mask_pattern);
    _cairo_pattern_get_extents(
        &mut (*extents).mask_pattern.base,
        &mut (*extents).mask,
        (*surface).is_vector() as cairo_bool_t,
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_composite_rectangles_init_for_stroke(
    mut extents: *mut cairo_composite_rectangles_t,
    mut surface: *mut cairo_surface_t,
    mut op: cairo_operator_t,
    mut source: *const cairo_pattern_t,
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_composite_rectangles_init_for_fill(
    mut extents: *mut cairo_composite_rectangles_t,
    mut surface: *mut cairo_surface_t,
    mut op: cairo_operator_t,
    mut source: *const cairo_pattern_t,
    ...
    _cairo_path_fixed_approximate_fill_extents(path, &mut (*extents).mask);
    status = _cairo_composite_rectangles_intersect(extents, clip);
    if status as libc::c_uint
        == CAIRO_INT_STATUS_NOTHING_TO_DO as libc::c_int as libc::c_uint
    {
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_composite_rectangles_init_for_polygon(
    mut extents: *mut cairo_composite_rectangles_t,
    mut surface: *mut cairo_surface_t,
    mut op: cairo_operator_t,
    mut source: *const cairo_pattern_t,
    ...
    _cairo_box_round_to_rectangle(&(*polygon).extents, &mut (*extents).mask);
    status = _cairo_composite_rectangles_intersect(extents, clip);
    if status as libc::c_uint
        == CAIRO_INT_STATUS_NOTHING_TO_DO as libc::c_int as libc::c_uint
    {
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_composite_rectangles_init_for_boxes(
    mut extents: *mut cairo_composite_rectangles_t,
    mut surface: *mut cairo_surface_t,
    mut op: cairo_operator_t,
    mut source: *const cairo_pattern_t,
    ...
    _cairo_box_round_to_rectangle(&mut box_0, &mut (*extents).mask);
    status = _cairo_composite_rectangles_intersect(extents, clip);
    if status as libc::c_uint
        == CAIRO_INT_STATUS_NOTHING_TO_DO as libc::c_int as libc::c_uint
    {
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_composite_rectangles_init_for_glyphs(
    mut extents: *mut cairo_composite_rectangles_t,
    mut surface: *mut cairo_surface_t,
    mut op: cairo_operator_t,
    mut source: *const cairo_pattern_t,
    ...
    mut clip: *const cairo_clip_t,
    mut overlap: *mut cairo_bool_t,
) -> cairo_int_status_t {
    let mut status: cairo_status_t = CAIRO_STATUS_SUCCESS;
    let mut int_status: cairo_int_status_t = CAIRO_INT_STATUS_SUCCESS;
    if _cairo_composite_rectangles_init(extents, surface, op, source, clip) == 0 {
        _cairo_composite_rectangles_fini(extents);
        return CAIRO_INT_STATUS_NOTHING_TO_DO;
    }
    ...
    if !overlap.is_null() && *overlap != 0
        && (*scaled_font).options.antialias as libc::c_uint
            == CAIRO_ANTIALIAS_NONE as libc::c_int as libc::c_uint
        && _cairo_pattern_is_opaque_solid(&mut (*extents).source_pattern.base) != 0
    {
        *overlap = 0 as libc::c_int;
    }
    int_status = _cairo_composite_rectangles_intersect(extents, clip);
    if int_status as libc::c_uint
        == CAIRO_INT_STATUS_NOTHING_TO_DO as libc::c_int as libc::c_uint
    {
        _cairo_composite_rectangles_fini(extents);
    }
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_composite_rectangles_can_reduce_clip(
    ...
    if (*composite).is_bounded
        & CAIRO_OPERATOR_BOUND_BY_SOURCE as libc::c_int as libc::c_uint != 0
    {
        _cairo_rectangle_intersect(&mut extents, &mut (*composite).source);
    }
    if (*composite).is_bounded
        & CAIRO_OPERATOR_BOUND_BY_MASK as libc::c_int as libc::c_uint != 0
    {
        _cairo_rectangle_intersect(&mut extents, &mut (*composite).mask);
    }
    _cairo_box_from_rectangle(&mut box_0, &mut extents);
    return _cairo_clip_contains_box(clip, &mut box_0);
}
#[no_mangle]
pub unsafe extern "C" fn _cairo_composite_rectangles_add_to_damage(
    mut composite: *mut cairo_composite_rectangles_t,
    mut damage: *mut cairo_boxes_t,
) -> cairo_int_status_t {
    let mut status: cairo_int_status_t = CAIRO_INT_STATUS_SUCCESS;
    let mut n: libc::c_int = 0;
    n = 0 as libc::c_int;
    while n < (*(*composite).clip).num_boxes {
        status = _cairo_boxes_add(
            damage,
            CAIRO_ANTIALIAS_NONE,
            &mut *((*(*composite).clip).boxes).offset(n as isize),
