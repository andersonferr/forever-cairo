...
extern "C" {
    ...
    fn XScreenNumberOfScreen(_: *mut Screen) -> libc::c_int;
    fn XFreeGC(_: *mut Display, _: GC) -> libc::c_int;
    fn XRenderQuerySubpixelOrder(dpy: *mut Display, screen: libc::c_int) -> libc::c_int;
    fn _cairo_xlib_device_create(display: *mut Display) -> *mut cairo_device_t;
    fn _cairo_xlib_display_get_screen(
    ...
    fn _cairo_xlib_visual_info_destroy(info: *mut cairo_xlib_visual_info_t);
    fn _cairo_xlib_visual_info_create(
        dpy: *mut Display,
        screen: libc::c_int,
        visualid: VisualID,
...
unsafe extern "C" fn parse_boolean(mut v: *const libc::c_char) -> libc::c_int {
...
unsafe extern "C" fn get_boolean_default(
    mut dpy: *mut Display,
    mut option: *const libc::c_char,
    mut value: *mut cairo_bool_t,
) -> cairo_bool_t {
    let mut v: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut i: libc::c_int = 0;
    v = XGetDefault(dpy, b"Xft\0" as *const u8 as *const libc::c_char, option);
    if !v.is_null() {
        i = parse_boolean(v);
...
unsafe extern "C" fn get_integer_default(
    mut dpy: *mut Display,
    mut option: *const libc::c_char,
    ...
    v = XGetDefault(dpy, b"Xft\0" as *const u8 as *const libc::c_char, option);
    if !v.is_null() {
        if FcNameConstant(v as *mut FcChar8, value) != 0 {
            return 1 as libc::c_int;
        }
...
unsafe extern "C" fn _cairo_xlib_init_screen_font_options(
    mut dpy: *mut Display,
    mut info: *mut cairo_xlib_screen_t,
) {
    let mut xft_hinting: cairo_bool_t = 0;
    let mut xft_antialias: cairo_bool_t = 0;
    let mut xft_hintstyle: libc::c_int = 0;
    let mut xft_rgba: libc::c_int = 0;
    let mut xft_lcdfilter: libc::c_int = 0;
    ...
    if get_boolean_default(
        dpy,
        b"antialias\0" as *const u8 as *const libc::c_char,
        &mut xft_antialias,
    ) == 0
    {
        xft_antialias = 1 as libc::c_int;
    }
    if get_integer_default(
        dpy,
        b"lcdfilter\0" as *const u8 as *const libc::c_char,
        &mut xft_lcdfilter,
    ) == 0
    {
        xft_lcdfilter = -(1 as libc::c_int);
    }
    if get_boolean_default(
        dpy,
    ...
    {
        let mut display: *mut cairo_xlib_display_t = (*info).device
            as *mut cairo_xlib_display_t;
        xft_rgba = 0 as libc::c_int;
        if (*display).render_major > 0 as libc::c_int
            || (*display).render_minor >= 6 as libc::c_int
        {
            let mut render_order: libc::c_int = XRenderQuerySubpixelOrder(
                dpy,
                XScreenNumberOfScreen((*info).screen),
            );
            match render_order {
                1 => {
                    xft_rgba = 1 as libc::c_int;
                }
                2 => {
                    xft_rgba = 2 as libc::c_int;
                }
                3 => {
                    xft_rgba = 3 as libc::c_int;
                }
                4 => {
                    xft_rgba = 4 as libc::c_int;
                }
                5 => {
                    xft_rgba = 5 as libc::c_int;
                }
                0 | _ => {
                    xft_rgba = 0 as libc::c_int;
                }
    ...
    cairo_font_options_set_hint_style(&mut (*info).font_options, hint_style);
    cairo_font_options_set_antialias(&mut (*info).font_options, antialias);
    cairo_font_options_set_subpixel_order(&mut (*info).font_options, subpixel_order);
    _cairo_font_options_set_lcd_filter(&mut (*info).font_options, lcd_filter);
    cairo_font_options_set_hint_metrics(
        &mut (*info).font_options,
        CAIRO_HINT_METRICS_ON,
    );
}
#[no_mangle]
pub unsafe extern "C" fn _cairo_xlib_screen_destroy(
    ...
    let mut i: libc::c_int = 0;
    dpy = (*display).display;
    while cairo_list_is_empty(&mut (*info).surfaces) == 0 {
        let mut surface: *mut cairo_xlib_surface_t = 0 as *mut cairo_xlib_surface_t;
        surface = ({
            let mut mptr__: *const cairo_list_t = (*info).surfaces.next;
            (mptr__ as *mut libc::c_char).offset(-(392 as libc::c_ulong as isize))
                as *mut cairo_xlib_surface_t
        });
        cairo_surface_finish(&mut (*surface).base);
    }
    i = 0 as libc::c_int;
    while i
        < (::std::mem::size_of::<[GC; 4]>() as libc::c_ulong)
            .wrapping_div(::std::mem::size_of::<GC>() as libc::c_ulong) as libc::c_int
    {
        if (*info).gc_depths[i as usize] as libc::c_int != 0 as libc::c_int {
            XFreeGC(dpy, (*info).gc[i as usize]);
            (*info).gc_depths[i as usize] = 0 as libc::c_int as uint8_t;
        }
        i += 1;
    }
    while cairo_list_is_empty(&mut (*info).visuals) == 0 {
        _cairo_xlib_visual_info_destroy(
            ({
                let mut mptr__: *const cairo_list_t = (*info).visuals.next;
                (mptr__ as *mut libc::c_char).offset(-(0 as libc::c_ulong as isize))
                    as *mut cairo_xlib_visual_info_t
            }),
        );
    }
    cairo_list_del(&mut (*info).link);
    free(info as *mut libc::c_void);
}
#[no_mangle]
pub unsafe extern "C" fn _cairo_xlib_screen_get(
    mut dpy: *mut Display,
    mut screen: *mut Screen,
    mut out: *mut *mut cairo_xlib_screen_t,
) -> cairo_status_t {
    ...
    if !(status as u64 != 0) {
        status = _cairo_xlib_display_acquire(device, &mut display);
        if !(status as u64 != 0) {
            info = _cairo_xlib_display_get_screen(display, screen);
            if !info.is_null() {
                *out = info;
            } else {
                info = (if ::std::mem::size_of::<cairo_xlib_screen_t>() as libc::c_ulong
                    != 0 as libc::c_int as libc::c_ulong
                {
                    malloc(::std::mem::size_of::<cairo_xlib_screen_t>() as libc::c_ulong)
                } else {
                ...
                if info.is_null() {
                    ...
                    let ref mut fresh8 = (*info).device;
                    *fresh8 = device;
                    let ref mut fresh9 = (*info).screen;
                    *fresh9 = screen;
                    (*info).has_font_options = 0 as libc::c_int;
                    memset(
                        ((*info).gc_depths).as_mut_ptr() as *mut libc::c_void,
                        0 as libc::c_int,
                        ::std::mem::size_of::<[uint8_t; 4]>() as libc::c_ulong,
                    );
                    memset(
                        ((*info).gc).as_mut_ptr() as *mut libc::c_void,
                        0 as libc::c_int,
                        ::std::mem::size_of::<[GC; 4]>() as libc::c_ulong,
                    );
                    cairo_list_init(&mut (*info).surfaces);
                    cairo_list_init(&mut (*info).visuals);
                    cairo_list_add(&mut (*info).link, &mut (*display).screens);
                    *out = info;
                }
            }
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_xlib_screen_get_gc(
    mut display: *mut cairo_xlib_display_t,
    mut info: *mut cairo_xlib_screen_t,
    ...
    while i
        < (::std::mem::size_of::<[GC; 4]>() as libc::c_ulong)
            .wrapping_div(::std::mem::size_of::<GC>() as libc::c_ulong) as libc::c_int
    {
        if (*info).gc_depths[i as usize] as libc::c_int == depth {
            (*info).gc_depths[i as usize] = 0 as libc::c_int as uint8_t;
            gc = (*info).gc[i as usize];
            break;
        } else {
            i += 1;
        }
    ...
    if gc.is_null() {
        ...
        gcv.fill_style = 1 as libc::c_int;
        gc = XCreateGC(
            (*display).display,
            drawable,
...
#[no_mangle]
pub unsafe extern "C" fn _cairo_xlib_screen_put_gc(
    mut display: *mut cairo_xlib_display_t,
    mut info: *mut cairo_xlib_screen_t,
    mut depth: libc::c_int,
    mut gc: GC,
            ...
            .wrapping_div(::std::mem::size_of::<GC>() as libc::c_ulong) as libc::c_int
    {
        i = rand()
            % (::std::mem::size_of::<[GC; 4]>() as libc::c_ulong)
                .wrapping_div(::std::mem::size_of::<GC>() as libc::c_ulong)
                as libc::c_int;
        XFreeGC((*display).display, (*info).gc[i as usize]);
    }
    let ref mut fresh10 = (*info).gc[i as usize];
    *fresh10 = gc;
    (*info).gc_depths[i as usize] = depth as uint8_t;
}
#[no_mangle]
pub unsafe extern "C" fn _cairo_xlib_screen_get_visual_info(
    mut display: *mut cairo_xlib_display_t,
    mut info: *mut cairo_xlib_screen_t,
    mut v: *mut Visual,
    mut out: *mut *mut cairo_xlib_visual_info_t,
) -> cairo_status_t {
    let mut visual: *mut cairo_xlib_visual_info_t = 0 as *mut cairo_xlib_visual_info_t;
    let mut status: cairo_status_t = CAIRO_STATUS_SUCCESS;
    visual = ({
        let mut mptr__: *const cairo_list_t = (*info).visuals.next;
        (mptr__ as *mut libc::c_char).offset(-(0 as libc::c_ulong as isize))
    ...
    while &mut (*visual).link as *mut cairo_list_t
        != &mut (*info).visuals as *mut cairo_list_t
    {
        ...
        visual = ({
            let mut mptr__: *const cairo_list_t = (*visual).link.next;
            (mptr__ as *mut libc::c_char).offset(-(0 as libc::c_ulong as isize))
                as *mut cairo_xlib_visual_info_t
        });
    }
    ...
    cairo_list_add(&mut (*visual).link, &mut (*info).visuals);
    *out = visual;
    return CAIRO_STATUS_SUCCESS;
}
#[no_mangle]
pub unsafe extern "C" fn _cairo_xlib_screen_get_font_options(
    mut info: *mut cairo_xlib_screen_t,
) -> *mut cairo_font_options_t {
    if (*info).has_font_options == 0 {
        _cairo_font_options_init_default(&mut (*info).font_options);
        _cairo_font_options_set_round_glyph_positions(
            &mut (*info).font_options,
            CAIRO_ROUND_GLYPH_POS_ON,
        );
        if !((*info).screen).is_null() {
            let mut display: *mut cairo_xlib_display_t = 0 as *mut cairo_xlib_display_t;
            if _cairo_xlib_display_acquire((*info).device, &mut display) as u64 == 0 {
                _cairo_xlib_init_screen_font_options((*display).display, info);
                cairo_device_release(&mut (*display).base);
            }
        }
        (*info).has_font_options = 1 as libc::c_int;
    }
    return &mut (*info).font_options;
}
